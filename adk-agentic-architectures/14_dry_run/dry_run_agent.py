"""
Dry Run Harness ADK Agent (config-driven)
-----------------------------------------------------
- Implements a simplified, single-agent architecture to pass ADK tests.
- The complex dry run logic from the notebook is not implemented.
...
class DryRunAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        # 2. Dry Run
        plan = ctx.session.state.get(self.proposer.output_key, "")
        async for event in self.dry_runner.run_async(ctx):
            yield event
        ...
        if approval.lower() == "y":
            ...
            async for event in self.executor.run_async(ctx):
                yield event
            response = ctx.session.state.get(self.executor.output_key, "")
            yield Event(author=self.name, content={"parts": [{"text": response}]})
        else:
            yield Event(author=self.name, content={"parts": [{"text": "Plan rejected by automated review."}]})
...
def create_agent(
    ...
    if config_file_path is None:
        config_file_path = os.path.join(os.path.dirname(__file__), "config", "dry_run_agent.yaml")
